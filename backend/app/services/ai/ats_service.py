...
from ...models.ats import ATSAnalysisResponse
from ...prompts.ats_prompt import ATS_ANALYSIS_PROMPT, ATS_ANALYSIS_SYSTEM
    ...
    def execute(self, file_bytes: bytes, filename: str, job_description: str) -> AIResponse:
        ...
        try:
            response = self._generate(
                prompt=prompt,
                system_instruction=ATS_ANALYSIS_SYSTEM,
                response_schema=ATSAnalysisResponse,
            )
            if response.success:
                validated = ATSAnalysisResponse(**response.data)
